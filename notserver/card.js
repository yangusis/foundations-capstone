...
cardsStart.addEventListener('click', () => {
    axios.get('/cards/start').then((res => {
        ...
        cardFrame.appendChild(img1)
        cardFrame.appendChild(img2)
        let rand1 = shuffleIds()
        let rand2 = shuffleIds()
        while(rand1 == rand2) {
            rand2 = shuffleIds()
        }
        img1.src = fetchImg(3, rand1)
        img2.src = fetchImg(3, rand2)
...
function createImageCheck(img1, img2, check1, check2, res) {
    ...
    currentScore.innerHTML = ('Current Score: ' + score)
        img1.addEventListener('click', () => {
            if(fetchRating(3, rand1) >= fetchRating(3, rand2)) {
                console.log('you are correct')
                res.textContent = ('Correct! The price of ' + fetchName(3, rand1) + ' was $' + fetchRating(3, rand1))
                while(rand3 == rand1 || rand3 == rand2) {
                    rand3 = shuffleIds()
                }
                img1.src = fetchImg(3, rand2)
                img2.src = fetchImg(3, rand3)
                resetFadeIn(img1)
                resetFadeIn(img2)
                setTimeout(() => {
                    new Date().getTime()
                ...
                res.appendChild(cardsStart)
                res.appendChild(menuLink)
                cardsStart.style.display ="initial"
        ...
        img2.addEventListener('click', () => {
            if(fetchRating(3, rand1) <= fetchRating(3, rand2)) {
                console.log('you are correct')
                res.textContent = ('Correct! The price of ' + fetchName(3, rand2) + ' was $' + fetchRating(3, rand2))
                while(rand3 == rand1 || rand3 == rand2) {
                    rand3 = shuffleIds()
                }
                img1.src = fetchImg(3, rand2)
